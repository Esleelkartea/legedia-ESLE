...
/**
 * Base class that represents a row from the 'usuario_grupo' table.
 *
 * 
 *
...
abstract class BaseUsuarioGrupo extends BaseObject  implements Persistent {
  ...
  const PEER = 'UsuarioGrupoPeer';
	...
	public function setIdUsuario($v)
	{
		...
		if ($this->id_usuario !== $v) {
			$this->id_usuario = $v;
			$this->modifiedColumns[] = UsuarioGrupoPeer::ID_USUARIO;
		}
	...
	public function setIdGrupo($v)
	{
		...
		if ($this->id_grupo !== $v) {
			$this->id_grupo = $v;
			$this->modifiedColumns[] = UsuarioGrupoPeer::ID_GRUPO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->id_grupo = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->resetModified();
    ...
    foreach (sfMixer::getCallables('BaseUsuarioGrupo:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseUsuarioGrupo:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseUsuarioGrupo:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseUsuarioGrupo:save:post') as $callable)
    {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsuarioGrupoPeer::ID_USUARIO)) $criteria->add(UsuarioGrupoPeer::ID_USUARIO, $this->id_usuario);
		if ($this->isColumnModified(UsuarioGrupoPeer::ID_GRUPO)) $criteria->add(UsuarioGrupoPeer::ID_GRUPO, $this->id_grupo);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseUsuarioGrupo:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseUsuarioGrupo::%s', $method));
    }
