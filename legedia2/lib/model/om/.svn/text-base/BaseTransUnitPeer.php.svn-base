...
abstract class BaseTransUnitPeer {
	...
	/**
	 * Selects a collection of TransUnit objects pre-filled with their Catalogue objects.
	 * @param      Criteria  $c
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	 * @return     array Array of TransUnit objects.
	 * @throws     PropelException Any exceptions caught during processing will be
	 *		 rethrown wrapped into a PropelException.
	 */
      ...
      call_user_func($callable, 'BaseTransUnitPeer', $c, $con);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (TransUnit) to $obj2 (Catalogue)
	...
	/**
	 * Selects a collection of TransUnit objects pre-filled with all related objects.
	 *
	 * @param      Criteria  $c
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
      ...
      call_user_func($callable, 'BaseTransUnitPeer', $c, $con);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (TransUnit) to the collection in $obj2 (Catalogue)
				$obj2->addTransUnit($obj1);
...
//
// NOTE: This static code cannot call methods on the TransUnitPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the TransUnitPeer class:
//
// Propel::getDatabaseMap(TransUnitPeer::DATABASE_NAME)->addTableBuilder(TransUnitPeer::TABLE_NAME, TransUnitPeer::getMapBuilder());
//
