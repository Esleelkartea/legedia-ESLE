...
class xmlResponse {
    ...
    static function connect() {
        ...
        //$opts = array('trace' => true, 'encoding'=>'ISO-8859-1', 'cache_wsdl' => WSDL_CACHE_NONE, 'local_cert' => './servidor.pem');
    ...
    static function probarXML($encodedXml) {
        ...
        if ($connect['result'] == 'fail')   return array('result' =>'Error de conexión', 'message' => $connect['message']);
        elseif ($connect['result'] == 'ok') {
            try {
                //_COMUNICACION_APD_
                /*$c1 = new Criteria();
                $c1->addAnd(ParametroPeer::TIPO_PARAMETRO, "_COMUNICACION_APD_", Criteria::EQUAL);
